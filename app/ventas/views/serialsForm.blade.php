	...
	new Vue({
		...
		methods: {
			...
			pushSerial: function(e)
			{
                ...
                $.ajax({
                    type: 'POST',
                    url: 'user/ventas/detalle_venta_serie_add',
                    data: { detalle_venta_id: ventas.detalleTable[{{$serial_index}}].id, serie:serie },
                }).done(function(data) {
                    if (!data.success)
                    	return msg.warning(data)

					that.serials.push(serie)
					ventas.detalleTable[{{$serial_index}}].serials.push(serie)
					$('#input_serie').val("")
                })
			},
			...
			removeSerial: function(index, serie)
			{
                ...
                $.ajax({
                    type: 'POST',
                    url: 'user/ventas/detalle_venta_serie_delete',
                    data: { detalle_venta_id: ventas.detalleTable[{{$serial_index}}].id, serie:serie },
                }).done(function(data) {
					...
					ventas.detalleTable[{{$serial_index}}].serials.$remove(index)
                })
			}
		}
	});
