...
@else
    <div class="container_selected" style="margin-left:10%">
        <div class="row">
            ...
            <div class="col-md-8">
                {{ Form::open(array('data-remote-abonosComprasPorSeleccion','onSubmit'=>'return false')) }}
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            {{ Form::select('metodo_pago_id', MetodoPago::where('id','!=',2)->where('id','!=',6)->where('id','!=',7)->lists('descripcion', 'id') ,'', array('class'=>'form-control col-md-6')) }}
                        </div>
        ...
        @foreach($compras as $q)
            <tr id="{{ $q->id }}">
                ...
                <td class="widthS center tbutton"  width="10%"> 
                    ...
                    <input type="checkbox" name="selectedPurshase[]" title="Seleccionar" total="{{ $q->saldo }}" value="{{ $q->id }}" class="select"> 
                </td>
            </tr>
        @endforeach
