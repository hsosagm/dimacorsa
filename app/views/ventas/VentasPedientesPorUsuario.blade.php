            ...
            <tbody>
                @foreach($ventas as $q)
                    <tr id="{{$q->id}}" class="{{($q->saldo_vencido > 0)? 'red':''}}">
                        ...
                        <td class="" width="20%"> {{ $q->usuario }} </td>
                        <td class="" width="20%"> {{ $q->tienda }} </td>
                        ...
                        <td class="center" width="5%">
                            <i id="{{$q->id}}" class="fa fa-plus-square btn-link theme-c" v-on="click: DetalleVentasPendientesPorUsuario($event, {{$q->id}})" ></i>
                        </td>
                    </tr>
                @endforeach
            </tbody>
